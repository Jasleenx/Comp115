...
def convert_american_dollars(american_dollars):
   canadian_dollar=american_dollars*1.34
   r=round(canadian_dollar,2)
   
   return r

convert_american_dollars(2)
...
def alarm_time(current_time_string, waiting_time_string):
  current_time = int(current_time_string)
  waiting_time = int(waiting_time_string)
...
def find_max(nums):
    ...
    return max_value
assert find_max ([1,2,5,9])==9    
assert find_max ([5,2,3,8])==8
...
assert evens ([9,8,7,4,5,6])==([8,4,6])            
assert evens ([8,6,4,2,1])==([8,6,4,2])      
...
assert sum_of_squares([1,4,9])==98
...
assert squares ([2,5,10])== [4,25,100] 
